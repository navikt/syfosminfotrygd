...
class SkalIkkeProdusereManuellOppgaveTest : FunSpec({
    context("Testing av metoden skalIkkeProdusereManuellOppgave") {
        test("Skal ikke oppdatere infotrygd når sykmelding er delvis overlappende med tidligere periode og sammenlagt periode for ny sykmelding er tre dager eller mindre") {
        ...
        test("Skal oppdatere infotrygd når sykmelding ikke overlapper med tidligere periode og sammenlagt periode for ny sykmelding er mer enn tre dager") {
            val validationResult = ValidationResult(
                status = Status.MANUAL_PROCESSING,
                ruleHits = listOf(
                    RuleInfo(
        ...
        test("Skal oppdatere infotrygd når sykmelding er delvis overlappende med tidligere periode og sammenlagt periode for ny sykmelding er mer enn tre dager") {
            val validationResult = ValidationResult(
                status = Status.MANUAL_PROCESSING,
                ruleHits = listOf(
                    RuleInfo(
