...
class OverstyrHoveddiagnoseTest :
    FunSpec({
        context("setHovedDiagnoseToA99IfhovedDiagnoseIsNullAndAnnenFraversArsakIsSet") {
            test(
                "Should set hoveddiagnose to A99 when hovedDiagnose is null and annenFraversArsak is not null "
            ) {
                ...
                defaultHealthInformation.medisinskVurdering.annenFraversArsak =
                    ArsakType().apply {
                        arsakskode.add(
                            CS().apply {
                                v = "2"
            ...
            test(
                "Should set not hoveddiagnose to A99 when hovedDiagnose is not null and annenFraversArsak is not null "
            ) {
                val defaultHealthInformation = createDefaultHealthInformation()
                defaultHealthInformation.medisinskVurdering.annenFraversArsak =
                    ArsakType().apply {
