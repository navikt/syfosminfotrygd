...
class ValidationTest :
    FunSpec({
        ...
        context("Testing validation rules and checking the rule outcomes") {
            ...
            test(
                "should trigger rule GRADERT_REISETILSKUDD_ER_OPPGITT, should be MANUAL_PROCESSING"
            ) {
                val sykmelding =
                    generateSykmelding(
                        perioder =
            ...
            test("should trigger rule PATIENT_NOT_IN_IP, should be MANUAL_PROCESSING") {
                ...
                val infotrygdForespResponse =
                    InfotrygdForesp().apply {
                        hovedStatus = StatusType().apply { kodeMelding = "00" }
                        pasient = InfotrygdForesp.Pasient().apply { isFinnes = false }
                    }
                ...
                result.first.treeResult.ruleHit shouldBeEqualTo
                    ValidationRuleHit.PATIENT_NOT_IN_IP.ruleHit
            ...
            test(
                "should trigger rule SICKLEAVE_EXTENTION_FROM_DIFFRENT_NAV_OFFICE_1, should be MANUAL_PROCESSING"
            ) {
                val generateSykmelding =
                    generateSykmelding(
                        perioder =
                ...
                result.first.treeResult.ruleHit shouldBeEqualTo
                    ValidationRuleHit.SICKLEAVE_EXTENTION_FROM_DIFFRENT_NAV_OFFICE_1.ruleHit

                result.first.treeResult.status shouldBeEqualTo Status.MANUAL_PROCESSING
            }

            test(
                "should trigger rule SICKLEAVE_EXTENTION_FROM_DIFFRENT_NAV_OFFICE_2, should be MANUAL_PROCESSING"
            ) {
                val generateSykmelding =
                    generateSykmelding(
                        perioder =
                ...
                result.first.treeResult.ruleHit shouldBeEqualTo
                    ValidationRuleHit.SICKLEAVE_EXTENTION_FROM_DIFFRENT_NAV_OFFICE_2.ruleHit

                result.first.treeResult.status shouldBeEqualTo Status.MANUAL_PROCESSING
            }

            test(
                "should trigger rule SICKLEAVE_EXTENTION_FROM_DIFFRENT_NAV_OFFICE_3, should be MANUAL_PROCESSING"
            ) {
                val generateSykmelding =
                    generateSykmelding(
                        perioder =
                ...
                result.first.treeResult.ruleHit shouldBeEqualTo
                    ValidationRuleHit.SICKLEAVE_EXTENTION_FROM_DIFFRENT_NAV_OFFICE_3.ruleHit

                result.first.treeResult.status shouldBeEqualTo Status.MANUAL_PROCESSING
            }

            test(
                "should trigger rule NEW_CLEAN_BILL_DATE_BEFORE_PAYOUT, should be MANUAL_PROCESSING"
            ) {
                val generateSykmelding =
                    generateSykmelding(
                        perioder =
                ...
                result.first.treeResult.ruleHit shouldBeEqualTo
                    ValidationRuleHit.NEW_CLEAN_BILL_DATE_BEFORE_PAYOUT.ruleHit

                result.first.treeResult.status shouldBeEqualTo Status.MANUAL_PROCESSING
            }

            test(
                "should trigger rule NEW_CLEAN_BILL_DATE_BEFORE_REGISTERD_CLEAN_BILL_DATE, should be MANUAL_PROCESSING"
            ) {
                val generateSykmelding =
                    generateSykmelding(
                        perioder =
                ...
                result.first.treeResult.ruleHit shouldBeEqualTo
                    ValidationRuleHit.NEW_CLEAN_BILL_DATE_BEFORE_REGISTERD_CLEAN_BILL_DATE.ruleHit
            ...
            test("should trigger rule EXTANION_OVER_FA, should be MANUAL_PROCESSING") {
                                    ...
                                    TypeSMinfo().apply {
                                        historikk.add(
                                            TypeSMinfo.Historikk().apply {
                                                tilltak =
                                                    TypeSMinfo.Historikk.Tilltak().apply {
                                                        type = "FA"
                ...
                result.first.treeResult.ruleHit shouldBeEqualTo
                    ValidationRuleHit.EXTANION_OVER_FA.ruleHit
            ...
            test("should trigger rule PERSON_MOVING_KODE_FL, should be MANUAL_PROCESSING") {
                ...
                result.first.treeResult.ruleHit shouldBeEqualTo
                    ValidationRuleHit.PERSON_MOVING_KODE_FL.ruleHit
            ...
            test("should trigger rule PERIOD_FOR_AA_ENDED, should be MANUAL_PROCESSING") {
                ...
                result.first.treeResult.ruleHit shouldBeEqualTo
                    ValidationRuleHit.PERIOD_FOR_AA_ENDED.ruleHit
            ...
            test("should trigger rule PERIOD_IS_AF, should be MANUAL_PROCESSING") {
                ...
                result.first.treeResult.ruleHit shouldBeEqualTo
                    ValidationRuleHit.PERIOD_IS_AF.ruleHit
            ...
            test("should trigger rule MAX_SICK_LEAVE_PAYOUT, should be MANUAL_PROCESSING") {
                ...
                result.first.treeResult.ruleHit shouldBeEqualTo
                    ValidationRuleHit.MAX_SICK_LEAVE_PAYOUT.ruleHit

                result.first.treeResult.status shouldBeEqualTo Status.MANUAL_PROCESSING
            }

            test(
                "should trigger rule ERROR_FROM_IT_HOUVED_STATUS_KODEMELDING, should be MANUAL_PROCESSING"
            ) {
                val generateSykmelding =
                    generateSykmelding(
                        perioder =
                ...
                val infotrygdForespResponse =
                    InfotrygdForesp().apply {
                        hovedStatus = StatusType().apply { kodeMelding = "5" }
                    }
                ...
                result.first.treeResult.ruleHit shouldBeEqualTo
                    ValidationRuleHit.ERROR_FROM_IT_HOUVED_STATUS_KODEMELDING.ruleHit

                result.first.treeResult.status shouldBeEqualTo Status.MANUAL_PROCESSING
            }

            test(
                "should trigger rule ERROR_FROM_IT_SMHISTORIKK_STATUS_KODEMELDING, should be MANUAL_PROCESSING"
            ) {
                val generateSykmelding =
                    generateSykmelding(
                        perioder =
                ...
                val infotrygdForespResponse =
                    InfotrygdForesp().apply {
                        ...
                        sMhistorikk =
                            InfotrygdForesp.SMhistorikk().apply {
                                status = StatusType().apply { kodeMelding = "5" }
                            }
                    }
                ...
                result.first.treeResult.ruleHit shouldBeEqualTo
                    ValidationRuleHit.ERROR_FROM_IT_SMHISTORIKK_STATUS_KODEMELDING.ruleHit

                result.first.treeResult.status shouldBeEqualTo Status.MANUAL_PROCESSING
            }

            test(
                "should trigger rule ERROR_FROM_IT_PARALELLYTELSER_STATUS_KODEMELDING, should be MANUAL_PROCESSING"
            ) {
                val generateSykmelding =
                    generateSykmelding(
                        perioder =
                ...
                val infotrygdForespResponse =
                    InfotrygdForesp().apply {
                        hovedStatus = StatusType().apply { kodeMelding = "00" }
                        parallelleYtelser =
                            InfotrygdForesp.ParallelleYtelser().apply {
                                status = StatusType().apply { kodeMelding = "5" }
                            }
                    }
                ...
                result.first.treeResult.ruleHit shouldBeEqualTo
                    ValidationRuleHit.ERROR_FROM_IT_PARALELLYTELSER_STATUS_KODEMELDING.ruleHit

                result.first.treeResult.status shouldBeEqualTo Status.MANUAL_PROCESSING
            }

            test(
                "should trigger rule ERROR_FROM_IT_DIAGNOSE_OK_UTREKK_STATUS_KODEMELDING, should be MANUAL_PROCESSING"
            ) {
                val generateSykmelding =
                    generateSykmelding(
                        perioder =
                ...
                result.first.treeResult.ruleHit shouldBeEqualTo
                    ValidationRuleHit.ERROR_FROM_IT_DIAGNOSE_OK_UTREKK_STATUS_KODEMELDING.ruleHit

                result.first.treeResult.status shouldBeEqualTo Status.MANUAL_PROCESSING
            }

            test(
                "should trigger rule ERROR_FROM_IT_PASIENT_UTREKK_STATUS_KODEMELDING, should be MANUAL_PROCESSING"
            ) {
                val generateSykmelding = generateSykmelding()
                ...
                result.first.treeResult.ruleHit shouldBeEqualTo
                    ValidationRuleHit.ERROR_FROM_IT_PASIENT_UTREKK_STATUS_KODEMELDING.ruleHit
